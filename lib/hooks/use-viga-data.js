...
export function usePerformanceData(limit = 200) {
    return useSWR(
        ['test_suites', {
            select: 'id, base_url, status, created_at, test_steps(created_at)',
...
export function useInfraStatus() {
    return useSWR(
        ...
        async () => {
            ...
            // Use server-side route to avoid exposing Groq API key to the browser
            const apiPromise = fetch('/api/live').catch(() => null);
            ...
            return [
                {
                    ...
                    status: apiResult?.ok ? (apiLatency < 2000 ? 'healthy' : 'warning') : 'error',
                    uptime: '99.99%',
                    load: `${apiLatency}ms`,
                    spec: 'Llama-3.3-70b',
                    type: 'Groq API',
