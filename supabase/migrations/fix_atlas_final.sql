-- FIX FINAL: Idempotent Schema for Atlas Agent
-- This script is safe to run multiple times. It handles "already exists" errors.
...
-- 3. Indexes (IF NOT EXISTS is standard in recent Postgres, but we can wrap in DO just to be safe or use simple IF NOT EXISTS)
...
-- Explicitly DROP before CREATE to avoid 42710 errors
