...
export default function StrategicHistory({ onReplay }) {
  ...
  if (loading) {
    return (
      <div className={`mt-12 flex flex-col items-center justify-center p-20 border rounded-[40px] ${isDark ? 'border-white/5 bg-black/20' : 'border-slate-200 bg-slate-50'
        }`}>
  ...
  if (history.length === 0) {
    return (
      <div className={`mt-12 border border-dashed rounded-[40px] p-24 text-center ${isDark ? 'border-white/10 bg-[#080808]' : 'border-slate-300 bg-white shadow-inner'
        }`}>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-6 items-center">
          <button
            ...
            className={`p-2 rounded-xl border flex items-center gap-2 text-[8px] font-black uppercase transition-all ${isDark ? 'border-white/5 hover:bg-red-500/10 text-slate-500 hover:text-red-500' : 'border-slate-200 hover:bg-red-50 text-slate-400 hover:text-red-600'
              }`}
          >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {history.map((run) => (
          <div
            ...
            className={`p-6 rounded-[32px] border transition-all group cursor-pointer relative overflow-hidden ${isDark
          ...
          >
            <div className="flex justify-between items-start mb-6 relative z-10">
              <div className={`p-2.5 rounded-xl ${run.status === 'completed' || run.status === 'success'
            ...
            <p className={`text-[11px] font-black truncate uppercase tracking-tight mb-2 relative z-10 ${isDark ? 'text-white' : 'text-slate-900'
              }`}>
              {run.name?.replace('SWARM:', '').trim() || 'Mission Alpha'}
            </p>

            <div className={`flex items-center gap-3 text-[9px] font-bold uppercase relative z-10 ${isDark ? 'text-slate-500' : 'text-slate-400'
              }`}>
              <Clock size={12} /> {new Date(run.created_at).toLocaleDateString()}
              <span>•</span>
              <span className={`px-2 py-0.5 rounded-full ${run.status === 'completed' ? 'text-emerald-500 bg-emerald-500/5' :
            ...
            <div className={`mt-5 h-[3px] w-full rounded-full overflow-hidden relative z-10 ${isDark ? 'bg-white/5' : 'bg-slate-100'
              }`}>
              <div
                className={`h-full transition-all duration-1000 ${run.status === 'completed' ? 'bg-emerald-500' : run.status === 'running' ? 'bg-blue-500 animate-pulse' : 'bg-red-500'
                  }`}
