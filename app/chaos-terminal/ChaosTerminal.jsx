...
export default function ChaosTerminal({ suiteId, open, onClose }) {
  ...
  const IT_FACTS = [
    "El primer bug de computadora fue una polilla real atrapada en un relé del Mark II en 1947.",
    "El código del Apolo 11 que nos llevó a la luna tenía menos líneas que un theme de WordPress moderno.",
    "El término 'Spam' viene de un sketch de Monty Python donde todo el menú tenía Spam.",
    "La primera cámara web se creó en Cambridge para vigilar si la cafetera tenía café.",
    "JavaScript fue creado en solo 10 días por Brendan Eich en 1995.",
    "El password más común del mundo sigue siendo '123456'.",
    "El primer dominio registrado fue symbolics.com el 15 de marzo de 1985.",
    "Linux corre en el 100% de las supercomputadoras más potentes del mundo.",
    "El logo de Android se inspiró en los símbolos de los baños públicos.",
    "Cada día se crean 2.5 quintillones de bytes de datos nuevos.",
    "El primer videojuego, 'Tennis for Two', se jugó en un osciloscopio en 1958."
  ];

  useEffect(() => {
    if (!selectedStep && status === 'running') {
      const interval = setInterval(() => {
        setCurrentFactIndex(prev => (prev + 1) % IT_FACTS.length);
      }, 5000);
            ...
            {selectedStep ? (
              ...
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
                <div className="mb-8 relative">
                  <Activity size={48} className={`animate-pulse duration-[2s] ${isDark ? 'text-blue-500' : 'text-blue-600'}`} />
                  <div className="absolute inset-0 animate-ping opacity-20 bg-blue-500 rounded-full blur-xl" />
                </div>

                <h3 className={`text-xs font-black uppercase tracking-[0.4em] mb-6 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                ...
                <AnimatePresence mode='wait'>
                  <motion.div
                    ...
                    className="max-w-md"
                  >
                    <p className={`text-sm font-medium leading-relaxed italic ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
                      "{IT_FACTS[currentFactIndex]}"
                    </p>
                  </motion.div>
                </AnimatePresence>
